...
-- Tables, triggers, functions, RLS policies.
-- Create Storage buckets "avatars" and "block-images" (public) in the dashboard before applying.
...
-- Time blocks: own + partner's (when matched), with explicit with check for inserts
...
-- Storage RLS policies (buckets avatars and block-images must exist)
